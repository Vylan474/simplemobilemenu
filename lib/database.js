...
async function createUser(userData) {
  const { id, email, passwordHash, name, restaurant, avatar, plan = 'free', maxMenus = 5 } = userData;
  
  try {
    const result = await sql`
      INSERT INTO users (id, email, password_hash, name, restaurant, avatar, plan, max_menus)
      VALUES (${id}, ${email}, ${passwordHash}, ${name}, ${restaurant || null}, ${avatar || null}, ${plan}, ${maxMenus})
      RETURNING *
    `;
