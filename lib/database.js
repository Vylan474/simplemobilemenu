...
async function initializeDatabase() {
  try {
    ...
    // Create uploaded_backgrounds table for tracking user uploads
    await sql`
...
async function createUser(userData) {
  ...
  try {
    const result = await sql`
      INSERT INTO users (id, email, password_hash, name, restaurant, avatar, google_id, plan, max_menus)
      VALUES (
        ${id}, 
        ${email}, 
        ${passwordHash || null}, 
...
async function saveMenuSections(menuId, sections) {
  try {
    console.log(`[DB] Saving ${sections.length} sections for menu ${menuId}`);
    
    // Delete existing sections
    const deleteResult = await sql`DELETE FROM menu_sections WHERE menu_id = ${menuId}`;
    console.log(`[DB] Deleted existing sections for menu ${menuId}`);
    
    // Insert new sections
    for (const section of sections) {
      console.log(`[DB] Inserting section: ${section.name} with ${section.items?.length || 0} items`);
      await sql`
    ...
    console.log(`[DB] Successfully saved all sections for menu ${menuId}`);
    return { success: true };
  } catch (error) {
